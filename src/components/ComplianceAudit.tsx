...
import { Scale, Loader2, FileText, Download } from 'lucide-react';
...
export function ComplianceAudit({ language }: { language: ProjectLanguage }) {
  ...
  const t = {
    es: {
      ...
      select: "Verifica GDPR, CCPA y estándares industriales",
      start: "INICIAR AUDITORÍA",
      checklist: "Checklist de Cumplimiento",
      high: "NON-COMPLIANT (ROJO)",
      mid: "GAP DETECTADO (AMARILLO)",
      low: "COMPLIANT (VERDE)",
      report: "AUDITORIA DE CUMPLIMIENTO AI",
      summary: "Reporte de Cumplimiento Normativo"
    },
    en: {
      title: "Compliance Audit",
      select: "Verify GDPR, CCPA, and industry standards",
      start: "START AUDIT",
      checklist: "Compliance Checklist",
      high: "NON-COMPLIANT (RED)",
      mid: "GAP DETECTED (YELLOW)",
      low: "COMPLIANT (GREEN)",
      report: "AI COMPLIANCE AUDIT",
  ...
  const handleAnalysis = async () => {
    ...
    try {
      ...
      const analysis = await analyzeLegalDocument(text, ['compliance_checker'], language);
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(22);
    doc.text("AUDITORIA DE CUMPLIMIENTO AI", 20, 25);
    
    y = 60;
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(14);
    doc.text("Reporte de Cumplimiento Normativo", 20, y);
    y += 10;
    doc.setFontSize(10);
    ...
    doc.save(`cumplimiento-${new Date().getTime()}.pdf`);
  };
        ...
        {result && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="flex justify-between items-center bg-black text-white px-8 py-4">
              <span className="font-black uppercase tracking-widest text-sm">{t.checklist}</span>
              <button 
                onClick={downloadPDF}
            ...
            <div className="legal-card p-4 bg-white grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex items-start gap-2">
                ...
                <p className="text-[10px] uppercase font-black tracking-tighter">{t.high}</p>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-3 h-3 bg-[#f1c40f] mt-1 brutalist-border" />
                <p className="text-[10px] uppercase font-black tracking-tighter">{t.mid}</p>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-3 h-3 bg-[#27ae60] mt-1 brutalist-border" />
                <p className="text-[10px] uppercase font-black tracking-tighter">{t.low}</p>
              </div>
            </div>
              ...
              <ReactMarkdown
                components={{
                  p: ({ children }) => {
                    ...
                    if (content.includes('[ALTO RIESGO]')) return <p className="mb-4 font-medium"><span className="risk-tag risk-high">{t.high.split(' ')[0]}</span>{content.replace('[ALTO RIESGO]', '')}</p>;
                    if (content.includes('[RIESGO MEDIO]')) return <p className="mb-4 font-medium"><span className="risk-tag risk-medium">{t.mid.split(' ')[0]}</span>{content.replace('[RIESGO MEDIO]', '')}</p>;
                    if (content.includes('[BAJO RIESGO]')) return <p className="mb-4 font-medium"><span className="risk-tag risk-low">{t.low.split(' ')[0]}</span>{content.replace('[BAJO RIESGO]', '')}</p>;
                    return <p className="mb-4 font-medium">{children}</p>;
                  },
                  li: ({ children }) => {
                    const content = React.Children.toArray(children).join('');
                    if (content.includes('[ALTO RIESGO]')) return <li className="pl-1 border-b border-black/10 py-1"><span className="risk-tag risk-high scale-75 origin-left">{t.high.split(' ')[0]}</span>{content.replace('[ALTO RIESGO]', '')}</li>;
                    if (content.includes('[RIESGO MEDIO]')) return <li className="pl-1 border-b border-black/10 py-1"><span className="risk-tag risk-medium scale-75 origin-left">{t.mid.split(' ')[0]}</span>{content.replace('[RIESGO MEDIO]', '')}</li>;
                    if (content.includes('[BAJO RIESGO]')) return <li className="pl-1 border-b border-black/10 py-1"><span className="risk-tag risk-low scale-75 origin-left">{t.low.split(' ')[0]}</span>{content.replace('[BAJO RIESGO]', '')}</li>;
